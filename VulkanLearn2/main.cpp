...
void Application::RecordCommandBuffer(const VkCommandBuffer& cmdBuffer, uint32_t imageIndex)
{
	...
	RenderImGui(cmdBuffer, imageIndex);
...
void Application::InitImGui()
{
	...
	VK_CHECK(vkCreateDescriptorPool(m_VulkanContext->getVulkanHandles().device, &pool_info, nullptr, &m_ImGuiDescriptorPool), "Lỗi tạo ImGui Pool");
	...
	init_info.Instance = m_VulkanContext->getVulkanHandles().instance;
	init_info.PhysicalDevice = m_VulkanContext->getVulkanHandles().physicalDevice;
	init_info.Device = m_VulkanContext->getVulkanHandles().device;
	init_info.QueueFamily = m_VulkanContext->getVulkanHandles().queueFamilyIndices.GraphicQueueIndex;
	init_info.Queue = m_VulkanContext->getVulkanHandles().graphicQueue;
...
// --- HÀM 2: VẼ IMGUI (Render Loop) ---
void Application::RenderImGui(VkCommandBuffer cmdBuffer, uint32_t imageIndex)
{
	// 1. New Frame
	...
	// 4. Ghi lệnh vẽ vào Command Buffer
...
void Application::ShutdownImGui()
{
	...
	vkDestroyDescriptorPool(m_VulkanContext->getVulkanHandles().device, m_ImGuiDescriptorPool, nullptr);
}
